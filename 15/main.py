...
ALL_BOOKS = []  #error: should be ALL_BOOKS instead of ALL_LIBRARY_BOOKS
ALL_LIBRARIES = []
DEBUG_MODE = True
GLOBAL_COUNTER = 0
...
class Library(Book):     #error: A library is not a book..Library should not inherit from Book
    ...
    def add_book(self, book):
        ...
        self.books2.append(book) #error: should add to self.books2 instead of self.books
    ...
    def __getitem__(self, index):  #error: should access self.books2 instead of ALL_BOOKS
        return ALL_BOOKS[index]

    def __len__(self):
        return len(self.books2)

    def __iter__(self):  #error:Iterating over one library returns books from all libraries.
        return iter(ALL_BOOKS)

    def __str__(self):    #error: __str__ should only return a string, not print.
